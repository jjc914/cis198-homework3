    ...
    F: Fn(&T, &HashMap<T, HashMap<T, E>>) -> Vec<(T /* dest vid */, E /* edge label */)>,
{
    ...
    to_visit: Vec<(T /* dest vid */, Option<E> /* edge label */)>,
}

impl<T, E, F> DFS<T, E, F> 
where
    T: Eq + PartialEq + Hash,
    E: Eq + PartialEq,
    F: Fn(&T, &HashMap<T, HashMap<T, E>>) -> Vec<(T /* dest vid */, E /* edge label */)>,
...
impl<T, E, F> Iterator for DFS<T, E, F>
where
    ...
    F: Fn(&T, &HashMap<T, HashMap<T, E>>) -> Vec<(T /* dest vid */, E /* edge label */)>,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let visiting = match self.to_visit.pop() {
            Some(v) => v,
            None => {
                return None;
            }
        ...
        if !self.visited.contains(&(visiting.0)) {
            for (adjacent, edge) in (self.next_adjacent)(&(visiting.0), &self.adjacency_map) { // get the adjacent 
                if !self.visited.contains(&adjacent) {
                    self.to_visit.push((adjacent, Some(edge)));
                }
            }
